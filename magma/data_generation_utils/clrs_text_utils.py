# The code in this file uses snippets or is adapted from the CLRS-Text Algorithmic Reasoning Language Benchmark.
# For more details, please refer to:
# Larisa Markeeva, Sean McLeish, Borja Ibarz, Wilfried Bounsi, Olga Kozlova, Alex Vitvitskyi,
# Charles Blundell, Tom Goldstein, Avi Schwarzschild, Petar Veličković. "The CLRS-Text Algorithmic
# Reasoning Language Benchmark", arXiv preprint arXiv:2406.04229, 2024.
...
def _create_input_feature_strs(spec, inputs):
...
def _create_output_feature_strs(spec, inputs, outputs):
...
def _convert_node_features_to_str(x, spec_name, spec, spec_type, inputs):
...
def _get_feature_by_name(examples, spec_name):
    filtered_inputs = [
...
def _get_output_name(hint_name: str, algo_name: str) -> str:
    ...
    if algo_name in CLRS_SEARCH_TAKS_OUTPUT_REPLACER:
        ...
        return hint_name.replace("_h", "")#hint_name[: -len(_HINT_PREFIX)]
...
def _get_output_str(sample, spec, algo_name, use_hints) -> list[str]:
...
def _do_not_include_input_in_text(spec_name, spec) -> bool:
    if spec_name == 'pos':
        return True
...
def clrs_text_sample(algo_name, sample, use_hints):
